...
typedef struct sio_port_initialization_block {
	...
	volatile uint32_t*	grpiereg;		/* モジュールストップコントロールレジスタ（MSTPCR） */
	...
	uint32_t			grpie_offset;	/* MSTPCRの対応するビットオフセット */
} SIOPINIB;
...
{
	{
		...
		(volatile uint32_t*)ICU_GENBL0_ADDR,
		INT_SCI6_TXI6,
		INT_SCI6_RXI6,
		INT_ICU_GROUPBL0,
		6,
	...
	(ID siopid, intptr_t exinf, uint8_t bitrate, uint8_t clksrc)
{
	...
	sil_wrw_mem(
			...
			(uint32_t)(sil_rew_mem((void *)p_siopcb->p_siopinib->grpiereg) | p_siopcb->p_siopinib->grpie_offset));
